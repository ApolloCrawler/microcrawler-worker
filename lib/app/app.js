...
exports.loadCrawlers = loadCrawlers;
...
function loadCrawlers() {
  return new Promise(function (resolve, reject) {
    var p = _path2.default.join(__dirname, '..', '..', 'node_modules');
    (0, _glob2.default)(p + '/microcrawler-crawler-*/package.json', function (err, paths) {
      if (err) {
        return reject(err);
      }

      var res = _ramda2.default.reject(function (item) {
        return item.includes('microcrawler-crawler-all') || item.includes('microcrawler-crawler-base');
      }, paths);

      res = _ramda2.default.map(function (item) {
      ...
      _ramda2.default.forEach(function (item) {
        ...
        _ramda2.default.forEach(function (processor) {
          map[item.name].processors[processor] = require(_path2.default.join(item.dir, item.pkg.crawler.processors[processor]));
...
var App = function () {
  ...
  _createClass(App, [{
    ...
    value: function main() {
      ...
      var crawlers = null;
      var promise = loadCrawlers().then(function (res) {
        crawlers = res;
      ...
      promise.then(function () {
        ...
        /* const channel = */createChannel(socket, channelName, _this.registerPingFunction.bind(_this), _this.unregisterPingFunction.bind(_this), crawlers);
      });
    }
  }]);
