...
function createChannel(socket, channelName, registerPingFunction, unregisterPingFunction, crawlers) {
  ...
  channel.on('crawl', function (data) {
    _winston2.default.info('Received event - crawl');
    ...
    try {
      return (0, _crawl2.default)(fetcher, crawlers, payload).then(function (result) {
...
function pingFunction(channel, id) {
  _winston2.default.info('Executing ping function.');
...
function createSocket(url, token, unregisterPingFunction) {
  _winston2.default.info('Connecting to "' + url + '"');
  ...
  socket.onError(function (err) {
    _winston2.default.error('There was an error with the connection!', err);
    unregisterPingFunction();
  });

  // Close handler
  socket.onClose(function () {
    _winston2.default.info('The connection dropped.');
    unregisterPingFunction();
  });
...
var Channel = function () {
  function Channel() {
    ...
    _winston2.default.info('Loading Supported Protocols');
  }
  ...
  _createClass(Channel, [{
    ...
    value: function registerPingFunction(channel) {
      ...
      _winston2.default.info('Registering ping function.');
    ...
    value: function unregisterPingFunction() {
      if (this.pingFunctionInterval) {
        _winston2.default.info('Unregistering ping function.');
