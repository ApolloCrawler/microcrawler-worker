...
function updateToken(username, password, urlAuth, tokenFilePath) {
  return new Promise(function (resolve, reject) {
    ...
    return _superagent2.default.post(urlAuth).send(payload).end(function (err, result) {
      ...
      _winston2.default.info('Storing token in ' + tokenFilePath);
      ...
      this.manager.loadCrawlers().then(function () {
        if (_commander2.default.crawl) {
          ...
          return (0, _crawl2.default)(fetcher, _this.manager.crawlers, payload).then(function (result) {
            _winston2.default.info(JSON.stringify(result, null, 4));
            process.exit(0);
          }, function (error) {
            _winston2.default.error(JSON.stringify(error, null, 4));
            process.exit(0);
          });
        }
        ...
        _winston2.default.error('Unable to load crawlers', err);
      }).then(function (newToken) {
        var token = newToken || _commander2.default.token || readToken();
        _winston2.default.info('Using token: ' + token);
        ...
        _winston2.default.error('Unable to updateToken', err);
      });
    }
  }, {
    key: 'channel',
