...
exports.DEFAULT_WORKERS_COUNT = exports.DEFAULT_HEARTBEAT_INTERVAL = exports.DEFAULT_TOKEN = exports.DEFAULT_LOG_LEVEL = exports.DEFAULT_CHANNEL = exports.DEFAULT_URL_AUTH = exports.DEFAULT_URL = exports.TOKEN = exports.TOKEN_PATH = undefined;
...
var DEFAULT_URL = exports.DEFAULT_URL = 'ws://localhost:4000';
...
var DEFAULT_LOG_LEVEL = exports.DEFAULT_LOG_LEVEL = 'warn';
...
function updateToken(username, password, urlAuth, tokenFilePath) {
  return new Promise(function (resolve, reject) {
    ...
    return _superagent2.default.post(urlAuth).send(payload).end(function (err, result) {
      ...
      _Logger2.default.info('Storing token in ' + tokenFilePath);
...
var App = function () {
  ...
  _createClass(App, [{
    ...
    value: function main() {
      ...
      .option('-u, --url <URL>', 'URL to connect to, default: ' + DEFAULT_URL + '/worker').option('-t, --token <TOKEN>', 'Token used for authorization, default: ' + DEFAULT_TOKEN).option('-a, --url-auth <URL>', 'URL used for authentication, default: ' + DEFAULT_URL_AUTH).option('--username <EMAIL>', 'Username').option('--password <PASSWORD>', 'Password').parse(args);
      ...
      this.manager.loadCrawlers().then(function () {
        if (_commander2.default.crawl) {
          ...
          return (0, _crawl2.default)(_this.manager.crawlers, payload).then(function (result) {
            _Logger2.default.info(JSON.stringify(result, null, 4));
            process.exit(0);
          }, function (error) {
            _Logger2.default.error(JSON.stringify(error, null, 4));
            process.exit(0);
          });
        }
        ...
        _Logger2.default.error('Unable to load crawlers', err);
      }).then(function (newToken) {
        var token = newToken || _commander2.default.token || readToken();
        _Logger2.default.info('Using token: ' + token);
        ...
        for (var i = 0; i < count; i += 1) {
          res.push(_this.channel.initialize(_commander2.default.url || DEFAULT_URL + '/worker', token, _commander2.default.channel || DEFAULT_CHANNEL, _this.manager));
        }

        return res;
      }, function (err) {
        _Logger2.default.error('Unable to updateToken', err);
      });
    }
  }, {
    key: 'channel',
